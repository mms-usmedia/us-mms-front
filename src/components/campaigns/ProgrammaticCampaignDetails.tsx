...
> = ({ campaign, onSave }) => {
  ...
  // Función para abrir el modal de confirmación de HUR
  const openHURModal = () => {
    setShowHURModal(true);
  };

  // Función para cerrar el modal de confirmación de HUR
  const closeHURModal = () => {
    setShowHURModal(false);
  };

  // Función para redirigir a la creación de un nuevo HUR
  const handleCreateHUR = () => {
    window.location.href = `/hur/new?campaignId=${campaign.id}`;
  };
          ...
          >
            <div className="text-center">
              ...
              <h3 className="mb-5 text-lg font-medium text-gray-900">
                Create HUR for this campaign?
              </h3>
              <p className="mb-5 text-sm text-gray-500">
                This action will initiate the Hold Until Resolved process for
                this campaign.
              </p>
              ...
              <div>
                ...
                {isEditing ? (
                  <input
                    ...
                    onChange={(e) => handleChange("trafficker", e.target.value)}
                    placeholder="Trafficker name"
                  />
                ) : (
                  <p className="mt-1 text-sm text-gray-900 font-medium bg-orange-50 px-2 py-1 rounded-md inline-block border border-orange-100">
                    {campaign.trafficker || "-"}
